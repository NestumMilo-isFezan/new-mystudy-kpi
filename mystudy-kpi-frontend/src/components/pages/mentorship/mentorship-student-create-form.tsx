...
import { StudentIdInput } from "@/components/pages/mentorship/manage-student/student-id-input";
...
type MentorshipStudentCreateFormProps = {
	defaultBatchId?: number;
};

export function MentorshipStudentCreateForm({
	close,
	payload,
}: ModalContentProps<MentorshipStudentCreateFormProps>) {
	const { quickCreateStudentMutation } = useMentorshipMutations();
	...
	const [formData, setFormData] = useState({
		...
		identifier: "",
		email: "",
		intakeBatchId: payload?.defaultBatchId ?? 0,
		password: "",
		confirmPassword: "",
	});
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		quickCreateStudentMutation.mutate(submitData, {
			onSuccess: () => close(),
		});
	};
		...
		<form onSubmit={handleSubmit} className="space-y-4">
			...
			<DialogFooter className="pt-4">
				<Button
					...
					variant="outline"
					onClick={close}
					disabled={quickCreateStudentMutation.isPending}
				>
					Cancel
				</Button>
				<Button type="submit" disabled={quickCreateStudentMutation.isPending}>
					{quickCreateStudentMutation.isPending
						? "Creating..."
						: "Save Student"}
				</Button>
			</DialogFooter>
